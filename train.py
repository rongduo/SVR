...
from gaussian_renderer import mpgs_render, network_gui
import sys
...
def pearson_correlation_coefficient(tensor1, tensor2, eps=1e-8):
    ...
    corr_coefficient = cov / (torch.sqrt(var1) * torch.sqrt(var2))
...
def training(dataset, opt, pipe, args):
    ...
    #estimate_depth = DepthEstimator().cuda()
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        midas_depth = 1.0 / torch.clamp(midas_depth , 1e-5, 1e3)
        midas_depth = (midas_depth - midas_depth.min()) / (midas_depth.max() - midas_depth.min())
        ...
        depth_loss = min(
            (1 - pearson_correlation_coefficient(-midas_depth, rendered_depth)),
            (1 - pearson_correlation_coefficient(1 / (midas_depth + 200.), rendered_depth))
        )
        ...
        with torch.no_grad():
            ...
            if  iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    ...
                    gaussians.compute_3D_filter(train_cams)
            ...
            if (iteration - args.start_sample_pseudo - 1) % opt.opacity_reset_interval == 0 and \
                ...
                gaussians.reset_opacity_with_filter3D()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[10_00, 20_00, 30_00, 50_00, 60_00, 70_00, 80_00, 10_000])
